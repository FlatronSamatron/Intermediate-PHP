...
- Исключения в PHP позволяют грамотно обрабатывать ошибки вместо прерывания скрипта. Они могут выбрасываться вручную или
...
- Исключения можно перехватывать с помощью блоков try-catch. Блок catch обрабатывает исключение и позволяет продолжить
  выполнение скрипта.
...
- Сообщения об ошибках определяются как свойства класса, чтобы избежать дублирования. Статические методы могут выступать
  в роли фабрик исключений.

- Глобальная функция обработки исключений set_exception_handler() позволяет обрабатывать все неперехваченные исключения.
